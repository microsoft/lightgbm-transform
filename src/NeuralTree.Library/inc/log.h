 ...
 * license information.
 */
#ifndef DynamicRank_UTILS_LOG_H_
#define DynamicRank_UTILS_LOG_H_
...
#define CHECK_NOTNULL(pointer)                                            \
  if ((pointer) == nullptr)                                               \
    DynamicRank::Log::Fatal(#pointer " Can't be NULL at %s, line %d .\n", \
                            __FILE__, __LINE__);
#endif
...
class Log {
 public:
  ...
  static void Fatal(const char *format, ...) {
    ...
    // otherwise to stderr
    // #ifndef LGB_R_BUILD
    fprintf(stderr, "[DynamicRank] [Fatal] %s\n", str_buf);
    fflush(stderr);
    // #else
    //     REprintf("[DynamicRank] [Fatal] %s\n", str_buf);
    //     R_FlushConsole();
    // #endif
  ...
  static void Write(LogLevel level, const char *level_str, const char *format,
    ...
    if (level <= GetLevel()) {  // omit the message with low level
      ...
      if (GetLogCallBack() == nullptr) {
        printf("[DynamicRank] [%s] ", level_str);
        vprintf(format, val);
        printf("\n");
        fflush(stdout);
      } else {
        ...
        char buf[kBufSize];
        snprintf(buf, kBufSize, "[DynamicRank] [%s] ", level_str);
        GetLogCallBack()(buf);
      ...
      // #else
      //       Rprintf("[DynamicRank] [%s] ", level_str);
      //       Rvprintf(format, val);
      //       Rprintf("\n");
      //       R_FlushConsole();
      // #endif
...
#endif  // DynamicRank_UTILS_LOG_H_
