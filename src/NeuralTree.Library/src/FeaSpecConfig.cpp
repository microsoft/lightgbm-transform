...
Config *Config::GetRawConfiguration(string str) {
    ...
    if (!line.empty()) {
      if (regex_match(line, result, sectionPattern)) {
        if (!sectionContent.empty()) {
          config.insert(map<string, map<string, string> >::value_type(
        ...
        sectionContent.insert(
            map<string, string>::value_type(result[1], result[2]));
  ...
  // Insert last section.
  config.insert(
      map<string, map<string, string> >::value_type(sectionId, sectionContent));
...
bool Config::DoesParameterExist(const char *section,
...
bool Config::GetStringParameter(const char *section, const char *parameterName,
                                string &value) {
  string sectionNameStr(section);
...
bool Config::GetDoubleParameter(const char *section, const char *parameterName,
                                double *value) {
  string sectionNameStr(section);
  string parameterNameStr(parameterName);
  map<string, string> sectionContent = _config[sectionNameStr];
...
double Config::GetDoubleParameter(const char *section,
                                  const char *parameterName,
                                  double defaultValue) {
  double value;
...
bool Config::GetBoolParameter(const char *section, const char *parameterName,
                              bool *value) {
  string sectionNameStr(section);
  string parameterNameStr(parameterName);
  map<string, string> sectionContent = _config[sectionNameStr];
