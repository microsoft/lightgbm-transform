...
void TestGetFeatureCount() {
  ...
  transform.reset(new TransformProcessor(
      Common::LoadStringFromFile(
          "./TestData/integration/SmoothedTrainInputIni"),
...
void TestTransformE2E() {
  ...
  std::unique_ptr<TransformProcessor> transform;
  transform.reset(new TransformProcessor(
      Common::LoadStringFromFile(input_ini_path.c_str()),
      Common::LoadStringFromFile(input_head_path.c_str(), 1), label_id));
  transformed_data =
      TransformedData::ToString(transform->ApplyForFile(input_data_path));
  ofstream output_fout(actual_output_path.c_str());
  for (auto str : transformed_data) output_fout << str << endl;
  output_fout.close();
