...
IniFileParserInterface *IniFileParserInterface::Createfromfreeform2(
    string freeform) {
  void *from_freeform2;
  try {
    from_freeform2 = InputExtractorCreateFromFreeformV2(freeform.c_str());
  } catch (const char *e) {
    Log::Fatal("Something wrong when loading Createfromfreeform2, detail:  %s",
               e);
  }
...
IniFileParserInterface *IniFileParserInterface::CreateFromInputStr(
    const string &str) {
  void *from_inputini;
  try {
    from_inputini = InputExtractorCreateFromInputStr(str);
  } catch (const char *e) {
    Log::Fatal("Something wrong when loading CreateFromInputStr, detail:  %s",
               e);
  }
...
void IniFileParserInterface::C_GetInputFeatures(void *input_ptr,
                                                uint32_t *features,
...
double IniFileParserInterface::C_EvaluateInput(void *input_ptr,
                                               uint32_t *input1) {
  double res;
  try {
...
void *IniFileParserInterface::C_GetInput(void *input_extractor_ptr,
                                         uint32_t index) {
  void *res;
  try {
...
bool IniFileParserInterface::C_GetInputName(void *input_extractor_ptr,
  ...
  bool res;
  try {
    res = InputExtractorGetInputName(input_extractor_ptr, feature_index, buffer,
                                     size_of_buffer, result_length);
  } catch (const char *e) {
...
bool IniFileParserInterface::C_GetFeatureName(void *input_extractor_ptr,
                                              uint32_t feature_index,
                                              char *buffer,
                                              uint32_t size_of_buffer,
                                              uint32_t *result_length) {
  bool res;
  try {
    res = FeatureMapGetFeatureName(input_extractor_ptr, feature_index, buffer,
                                   size_of_buffer, result_length);
  } catch (const char *e) {
...
void IniFileParserInterface::GetInputFeatures(void *input_ptr,
                                              uint32_t *features,
                                              uint32_t size_of_features,
                                              uint32_t *feature_count) {
  return IniFileParserInterface::C_GetInputFeatures(
      input_ptr, features, size_of_features, feature_count);
}
...
double IniFileParserInterface::EvaluateInput(void *input_ptr,
                                             uint32_t *input1) {
...
string IniFileParserInterface::GetInputName(uint32_t input_id) {
  return this->StringFetch(&IniFileParserInterface::C_GetInputName,
                           (uint32_t)input_id, this->input_extractor_ptr_);
}

string IniFileParserInterface::GetFeatureName(uint32_t input_id) {
  return this->StringFetch(&IniFileParserInterface::C_GetFeatureName,
                           (uint32_t)input_id, this->feature_map_ptr_);
}

string IniFileParserInterface::StringFetch(
    bool (*fetcher)(void *input_extractor_ptr, uint32_t feature_index,
                    char *buffer, uint32_t size_of_buffer,
                    uint32_t *result_length),
    uint32_t id, void *object_ptr) {
  std::vector<unsigned char> array(100);
  uint32_t num = 0;
  uint32_t *num_ptr = &num;

  if (!(*fetcher)(object_ptr, id, reinterpret_cast<char *>(&array[0]),
                  (uint32_t)array.size(), (uint32_t *)num_ptr)) {
...
IniFileParserInterface::IniFileParserInterface(void *p_input_extractor) {
  ...
  feature_map_ptr_ =
      this->IniFileParserInterface::GetFeatureMap(input_extractor_ptr_);
}
