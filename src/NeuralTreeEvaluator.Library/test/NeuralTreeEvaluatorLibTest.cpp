...
void EvaluateBasicInputs(string fea_spec_path) {
  DynamicRank::Config *config = DynamicRank::Config::GetRawConfiguration(
      Common::LoadStringFromFile(fea_spec_path.c_str()));
  assert(config->DoesSectionExist("Input:3"));
  InputExtractor *extractor = InputExtractorCreateFromInputStr(
      Common::LoadStringFromFile(fea_spec_path.c_str()));
...
void EvaluateStandaloneFreeFormV2() {
  InputExtractor *extractor =
      InputExtractorCreateFromFreeformV2("(if (== Foo Bar) 1 0)");
