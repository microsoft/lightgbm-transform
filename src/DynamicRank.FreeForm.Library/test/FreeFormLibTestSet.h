...
class FreeFormLibTestSet {
  ...
  static void AssertSzAreEqual(const char *cptr0, const char *cptr1) {
    ...
    while (*(cptr0 + offset) && *(cptr1 + offset)) ++offset;
  ...
  // Test base:
  static void TestInput(DynamicRank::Config &p_config, double p_result,
                        const char *p_serial, const char *p_section) {
    SimpleFeatureMap map;
    FreeForm2::CompiledNeuralInputLoader<FreeForm2::NeuralInputFreeForm2>
        loader("FreeForm2");
    std::auto_ptr<DynamicRank::NeuralInput> input(
        loader(p_config, p_section, map));
 ...
 public:
  ...
  static void TestParser(const char *freeform) {
    ...
    boost::shared_ptr<FreeForm2::NeuralInputFreeForm2> input =
        boost::shared_ptr<FreeForm2::NeuralInputFreeForm2>(
            new FreeForm2::NeuralInputFreeForm2(std::string(freeform),
                                                "freeform2", *featureMap));
    AssertIsNotNull(input.get());
